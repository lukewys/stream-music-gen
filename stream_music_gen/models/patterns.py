# Copied from https://github.com/facebookresearch/audiocraft/blob/main/audiocraft/modules/codebooks_patterns.py
...
LayoutCoord = namedtuple(
    "LayoutCoord", ["t", "q"]
)  # (timestep, codebook index)
...
class Pattern:
    ...
    def get_sequence_coords_with_timestep(
    ...
    def get_steps_with_timestep(
        ...
        return [
            step
            for step, coords in self.get_sequence_coords_with_timestep(t, q)
        ]
    ...
    def get_first_step_with_timesteps(
        ...
        return (
            steps_with_timesteps[0] if len(steps_with_timesteps) > 0 else None
        )
    ...
    def revert_pattern_logits(
        ...
        values = logits[:, :, indexes.reshape(-1)]
...
class UnrolledPatternProvider(CodebooksPatternProvider):
    ...
    def __init__(
        ...
        self._flattened_codebooks = self._build_flattened_codebooks(
            delays, flattening
        )
    ...
    def _num_inner_steps(self):
        ...
        return (
            max([inner_step for inner_step in self._flattened_codebooks.keys()])
            + 1
        )
        ...
        for t in range(max_timesteps):
            ...
            for step in range(self._num_inner_steps):
                if step in self._flattened_codebooks:
                    ...
                    coords = [
                        LayoutCoord(t, q) for q in step_codebooks.codebooks
                    ]
