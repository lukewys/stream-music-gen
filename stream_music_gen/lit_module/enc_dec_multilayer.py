...
from stream_music_gen.models.models_multi_out import (
    EncoderDecoderTransformerMultiOut,
)
...
EncoderDecoderTransformerMultiOut = bind(EncoderDecoderTransformerMultiOut)
...
class LitEncoderDecoderMultiOut(BaseLightningModel):
    def __init__(
        self,
        compile: bool = True,
        ...
        """
        Lightning Module for Encoder/Decoder, multilayer/multiout.

        Attributes:
            inst_tokens_as_pattern_token: if we are adding inst_tokens to
            inputs/targets

        All other attributes are the same as LitEncoderDecoder
        """
        super(LitEncoderDecoderMultiOut, self).__init__()
        ...
        self.model = EncoderDecoderTransformerMultiOut(
            enc_num_tokens=self.num_tokens,
            dec_num_tokens=self.num_tokens,
            enc_max_seq_len=self.max_gen_seq_len
            + add_inst_tokens,  # +1 for the instrument token, no bos/eos
            dec_max_seq_len=self.max_gen_seq_len
            + add_inst_tokens
            + 1,  # +1 for instrument, +1 for patterning
    ...
    def training_step(self, batch, batch_idx):
        ...
        logits, logits_mask = self.model(
            ...
            dec_inst_tokens=dec_inst_tokens,
        )
