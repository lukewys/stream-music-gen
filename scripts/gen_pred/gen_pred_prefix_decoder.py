"""Generate model predictions using the prefix decoder model."""
...
from stream_music_gen.lit_module.prefix_dec_multilayer import (
    LitPrefixDecoderMultiOut,
)
...
def generate_prediction(
    batch: any,
    model: LitPrefixDecoderMultiOut,
    ...
    assert not (
        random_input and zero_input
    ), "Cannot have both random and zero input"

    input_token_stems = batch["input_token_stems"]
    input_emb = batch["input_emb"].to(device)
    dec_inputs = batch["target_token"].to(device)
    ...
    # Add true padding for the first token
    ...
    if prompt_secs > 0:
        ...
        seq_out_start = batch["target_token"][:, :, :prompt_frames].to(device)
    else:
        seq_out_start = None
...
@bind
def main(
    model_path: str = "online-stem-gen/logs/prefix_decoder/step=100000.ckpt",
    save_dir: str = "online-stem-gen/model_predictions/model_predictions_prefix_decoder",
    ...
    model, tokenizer, dataloaders = load_lit_model(
        model_path,
        lit_module_cls=LitPrefixDecoderMultiOut,
